  ...
  if(isset($_POST['edit_data'])){
    ...
    if($nama_file != '')
    {
      ...
      $update = mysqli_query($koneksi, "UPDATE tb_pengajuan SET id_pengajuan='$id_pengajuan',npm='$npm',alasan='$alasan',
                ...
                ttd_ortu='$nama_file',semester='$semester', ,no_sk='$no_sk' WHERE id_pengajuan='$id_pengajuan'");

      if($update) {
        echo "<script> 
              alert ('Data Berhasil di Update'); window.location = 'data_verifikasi.php' </script> ";
      }
      else 
      {
        echo "<script> 
              alert ('Data Gagal di Update'); window.location = 'data_verifikasi.php' </script> ";
      }
    }
    else
    {
      $update = mysqli_query($koneksi, "UPDATE tb_pengajuan SET id_pengajuan='$id_pengajuan',npm='$npm',alasan='$alasan',
                tgl_pengajuan='$tgl_pengajuan',nama_ortu='$nama_ortu',status_pengajuan='$status_pengajuan',semester='$semester',no_sk='$no_sk' WHERE id_pengajuan='$id_pengajuan'");
