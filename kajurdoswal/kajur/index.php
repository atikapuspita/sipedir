...
<?php
 ...
 $user = mysqli_query($koneksi, "SELECT * FROM tb_kajur join tb_pegawai on tb_pegawai.id_pegawai=tb_kajur.id_pegawai WHERE tb_kajur.id_pegawai = '$id_pegawai'");
...
<?php
    $data_pengajuan = mysqli_query($koneksi, "SELECT * FROM tb_pengajuan INNER JOIN tb_mahasiswa ON tb_pengajuan.id_mahasiswa = tb_mahasiswa.id_mahasiswa INNER JOIN tb_kelas on tb_kelas.id_kelas=tb_mahasiswa.id_kelas join tb_prodi on tb_prodi.id_prodi=tb_kelas.id_prodi WHERE status_pengajuan = '3' AND tb_prodi.id_jurusan = '$id_jurusan';");
