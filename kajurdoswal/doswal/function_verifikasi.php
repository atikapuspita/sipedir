...
function verifikasi($data){
    ...
    $id_doswal = htmlspecialchars($data["id_doswal"]);
    ...
    if($status_verifikasi == "Ditolak") {
        ...
        $query = "INSERT INTO 
        `tb_verifikasi_doswal`
        ( `id_pengajuan`, `tgl_verifikasi`, `id_doswal`, `status_verifikasi`, `status_pengajuan`, `alasan_tolak`) 
        VALUES ('$id_pengajuan',NOW(),'$id_doswal','$status_verifikasi','5', '$alasan_tolak')";
        ...
        $query = "INSERT INTO 
        `tb_verifikasi_doswal`
        ( `id_pengajuan`, `tgl_verifikasi`, `id_doswal`, `status_verifikasi`, `status_pengajuan`) 
        VALUES ('$id_pengajuan',NOW(),'$id_doswal','$status_verifikasi','1')";
        ...
        if(!$result){
            ...
            $get_pengajuan = mysqli_query($koneksi,"SELECT * FROM tb_pegawai inner join tb_unit_kerja on tb_unit_kerja.id_pegawai=tb_pegawai.id_pegawai where jabatan ='4' AND tb_unit_kerja.status='1'");
            $data_pengajuan = mysqli_fetch_assoc($get_pengajuan);
            $nomorhp = $data_pengajuan['no_telp_pegawai'];
            // echo    $nomorhp;
            if($nomorhp!=null){
                $phone= preg_replace('/^0?/','62', $nomorhp); 
                    $message = "Terdapat pengajuan pengunduran diri mahasiswa yang telah disetujui dosen wali, segera cek SIAKAD";
