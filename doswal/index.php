...
<?php
 ...
 $user = mysqli_query($koneksi, "SELECT * FROM tb_doswal join tb_pegawai on tb_pegawai.id_pegawai=tb_doswal.id_pegawai WHERE tb_doswal.id_pegawai = '$id_pegawai'");
 ...
 $id_kelas = $tik["id_kelas"];
// echo $id_kelas;
 ?>

<?php
    $data_pengajuan = mysqli_query($koneksi, "SELECT * FROM tb_pengajuan INNER JOIN tb_mahasiswa ON tb_pengajuan.id_mahasiswa = tb_mahasiswa.id_mahasiswa WHERE status_pengajuan = '0' AND id_kelas = '$id_kelas';");
    $jumlah_pengajuan = mysqli_num_rows($data_pengajuan);
?>

<?php
    $data_verifikasi = mysqli_query($koneksi, "SELECT * FROM tb_verifikasi_doswal WHERE status_pengajuan = '1'");
