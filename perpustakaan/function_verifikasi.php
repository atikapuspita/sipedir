...
function verifikasi($data){
    ...
    if($status_verifikasi == "Ditolak") {
        ...
        VALUES ('$id_pengajuan',NOW(),'$id_unit','$status_verifikasi','2')";
        ...
        if(!$result){
            ...
            $get_pengajuan = mysqli_query($koneksi,"SELECT * FROM tb_pegawai inner join tb_unit_kerja on tb_unit_kerja.id_pegawai=tb_pegawai.id_pegawai where jabatan ='3' and tb_unit_kerja.status='1' ");
            $data_pengajuan = mysqli_fetch_assoc($get_pengajuan);
            $nomorhp = $data_pengajuan['no_telp_pegawai'];
            // echo    $nomorhp;
            if($nomorhp!=null){
                $phone= preg_replace('/^0?/','62', $nomorhp); 
                    $message = "Terdapat pengajuan pengunduran diri mahasiswa yang telah disetujui bagian perpustakaan, segera cek SIAKAD";
